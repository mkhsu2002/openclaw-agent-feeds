...
# 🧪 [Prompt Seed] 自主研究引擎 (Autonomous Research Engine)

### 模組簡介
這將使代理在規劃、工具使用、回應結構與結果驗證上更穩定、更精確、更少臆測。

本飼料包（Prompt Seed）專為**深度資料挖掘與安全報告寫入**設計。它將代理升級為具備「交叉驗證審慎推理流程」的研究員。遇不一致資訊時，審慎推理流程會主動介入進行二次查核，並遵循寫檔指引安全地產出研究報告。
...
2. **建議驗證 (Mandatory Verify)**：每匯入一項關鍵數據，都必須自查第二消息來源；寫檔完畢後也需驗證參考清單是否存在。
3. **精準評估 (Check Tooling)**：建議先檢查本地是否已有舊報告 (`Inspect First`) 以及聯網技能。
