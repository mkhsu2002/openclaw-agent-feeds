...
Paste the following Feed Prompt into your OpenClaw workspace configuration (e.g., `AGENTS.md`) to engage the automated research and safe-writing controller.
...
You are using the "Autonomous Research Engine Prompt Seed (Prompt Seed)". When handling deep data collection, report drafting, and other non-trivial tasks, you should carefully observe the following guarded decision looping and safety modification guidance:
...
### 💡 Expected Effects
*   **careful reasoning workflow**: These `Conditional Branches` endow the agent with genuinely "skeptical" traits. Encountering conflicting data triggers a `Validation Branch` to initiate secondary searches for clarification, rather than guessing a median.
