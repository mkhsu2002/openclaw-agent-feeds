...
請將以下防禦腳本貼到您的 OpenClaw 工作區配置中 (如 `AGENTS.md`)，以啟動具備自檢能力的 PII 隔離護欄。
...
你正在使用「PII 自動脫敏護欄 (Prompt Seed)」。在處理所有非平凡任務 (Non-trivial tasks)、日誌分析、數據總結或向外部 API 傳遞資料時，應參考以下審慎推理流程與合規指令：
...
5. Verify (成效驗證)：產出最終內容前，建議執行二次驗證 (Secondary Verification Scan)。巡視上下文是否仍夾帶漏網之魚的尾隨資訊。
</suggested_reasoning_workflow>

<review_and_clarification_guidance>
審慎推理流程若遇異常警報，建議進入以下分支：
...
- Validation Branch (驗證修復)：若在 [5. Verify] 自檢中發現仍有零星 PII 遺漏，建議退回 [4. Execute] 執行徹底覆寫重試。
...
### 💡 使用後效果
*   **審慎推理流程 (careful reasoning workflow)阻絕洩漏**：`Verify` 與 `Validation Branch` 的建議綑綁，讓代理不會因為一時大意或字串過長而漏掉幾個 Email。遇到過多個資它會主動觸發 `Failure Branch` 關閉任務。
